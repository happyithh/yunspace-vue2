<template>
    <div>
        <form action="" id="submit-demand">
            <div class="evformbox">
                <h2 v-if="!isOneKey">我要办活动</h2>
                <h2 v-if="isOneKey">一键租场地</h2>
                <div class="mainbox">
                    <ul class="inputbox">
                        ...
                        <li class="inputwrap clearfix">
                            ...
                            <button type="button" id="sendCode" class="sendcode" @click="sendPhoneCode($event)">发送验证码</button>
                        </li>
                        <li class="inputwrap clearfix">
                    ...
                    <div class="caption">
                        <p>一键提交你的活动需求</p>
                        <p v-if="isWordTime">我们将在30分钟内您提供多份精准场地报价</p>
                        <p v-if="!isWordTime">我们将尽快提供精准场地报价</p>
                    </div>
                </div><!--表单mainbox-end-->
            </div>
        </form>
...
//                var self = this
//                if(data.name){
//                    self.demand.phone = data.mobile
//                    self.demand.contact = data.name
//                }
                return data
            }
        },
        mounted () {
            ...
            setTimeout(function () {
                if(self.personalData.name){
                    self.demand.phone = self.personalData.mobile
                    self.demand.contact = self.personalData.name
                }
            },300)

            var now = new Date(),
                    hour = now.getHours();
            if(hour > 9 && hour < 18){this.isWordTime=true}else{this.isWordTime=false}

            if(typeof this.$route.query.type != 'undefined' && this.$route.query.type == 'onekey'){
                this.isOneKey = true
            }
...
//                            minlength : 11,
            ...
            if(typeof this.$route.query.activity_type != 'undefined'){
                this.demand.activity_type = this.$route.query.activity_type
            }
...
//                }
                ...
                self.demand.order_city = $('#cityId input').val() // 需求单城市
...
//                                }
                                ...
                                if (self.personalData.uid) { // 本地验证是否登陆
            ...
            sendPhoneCode : function (e) {
                ...
                GlobleFun.sendPhoneCode(self.demand.phone,success,e.target);
            },
        }
