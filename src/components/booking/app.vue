  ...
  <div>
    ...
    <div class="w1200 booking-container clearfix">
        ...
        <div class="fl booking-left">
            <h2>{{spaceDtl.name}}</h2>
            <p>空间类型：{{spaceDtl.site_type}}</p>
            <p>区域位置：{{spaceDtl.address}}</p>
            <div class="img">
                <img :src="spaceDtl.img_paths.length > 0 ? spaceDtl.img_paths[0]['url_400_267'] : ''">
            </div>
            <div class="pgroup clearfix">
                <p>最大容纳：{{spaceDtl.Max_seating_capacity}} 人</p>
                <p>空间面积：{{spaceDtl.area}} ㎡</p>
                <p>空间层高：{{spaceDtl.height}} m</p>
                <p>空间承重：{{spaceDtl.accept_weight}} m</p>
            </div>

            <!--<h3>周一至周四</h3>-->
            <div class="price">
                <p class="currentprice red">{{spaceDtl.special_price}}</p>
                <p class="costprice" v-if="spaceDtl.market_price != '面议'">{{spaceDtl.market_price}}</p>
            </div>
            <div class="text">
                <h6>套餐信息</h6>
                <p>
...
//            },
        ...
        methods: {
            ...
            submitInquiry : function () {
                ...
                this.inquiryData.order_city = this.spaceDtl.city_name
...
//                        console.log(data);
            ...
            getSpaceDtl : function () {

                var self= this;
//                self.$store.commit('loading',true);
                $.get({
                    url: window.YUNAPI.spaceDtlOnly,
