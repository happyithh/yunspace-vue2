...
window.router = new VueRouter({
    ...
    routes: [
        ...
        {
            path: '/search/dtl/:id',
            component: (resolve) => require(['./components/search/dtl'], resolve)
        },
        {
            path: '/search/map',
            component: (resolve) => require(['./components/search/map-search'], resolve)
        },
...
window.store = new Vuex.Store({
    ...
    mutations : {
        ...
        cityHotKeyword(state){
            setTimeout(function () {
                $.ajax({
                    url: window.YUNAPI.cityHotKeyword, data : {city_id:state.city_id}, success: function (data) {
                        state.cityHotKeyword = data
                    }
                });
            },200)
        },
        spaceManageChange(state,value){
            LS.set('spaceManageSelect',value)
            if(value.length){
                state.spaceManageSelectCount = value.length
            }else{
                state.spaceManageSelectCount = 0
            }
            state.spaceManageSelectList = value
        },
        getSpaceManageList(state){
            let array = LS.get('spaceManageSelect');
            if(!array){
                array = []
            }
            state.spaceManageSelectList = array
            state.spaceManageSelectCount = array.length
        },
        changeShowHeader(state,value){
            state.showHeader = value
        },
        changeShowFooter(state,value){
...
//                            minlength : 11,
    ...
    httpError (){
        ...
        window.store.commit('loading',false);
    },
    objConcat(a,b){
        return $.extend({}, a,b);
    },
